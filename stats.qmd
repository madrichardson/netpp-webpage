...
This metric alls for sensor-to-sensor consistency assessment, ensuring that variations are not biased by absolute magnitude differences.

### Biased Relative Difference (&Delta;<sub>rel</sub>)

The biased relative difference (&Delta;<sub>rel</sub>) compares the interim VIIRS netPP product (netPP<sub>VIIRS</sub>) and legacy MODIS netPP product (netPP<sub>MODIS</sub>) estimates, using MODIS as the reference. This approach helps determine whether VIIRS systematically overestimates or underestimates netPP compared to MODIS.

The pixel-by-pixel mean biased relative difference (&Delta;<sub>rel</sub>) was created for the globe for each month at 9km resolution across [two datasets.](#download-statistical-analysis-data-products)
