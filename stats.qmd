...
::: {style="text-align: center;"}
![](images/modis_viirs_corr_comparison.png){width=100%}
**Figure 5.**  Global maps of the Pearson Correlation coefficient calculated between MODIS-Aqua with VIIRS-SNPP (top) and VIIRS-NOAA20 (bottom). The color scale ranges from -1.0 (red, strong negative correlation) to 1.0 (blue, strong positive correlation).
:::

The majority of both maps are dark blue (close to r = 1.0) (**Fig. 5**), indicating MODIS and VIIRS-based sensors show strong positive correlations in these areas. Both VIIRS sensors agree with MODIS in estimating primary productivity throught most of the globe. 

Areas such as upwelling regions and polar regions appear lighter blue, indicating weaker agreement between the sensors. MORE INFO HERE
